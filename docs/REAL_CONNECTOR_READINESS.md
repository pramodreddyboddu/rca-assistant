...
| Queue depth / channel status / listener status / config via PCF inquire | **Live-verified** 2026-09-21 (QM1, MQ 10.0.0.5); fakes + replay fixtures in our suite |
| Error-log tail + AMQERR parse | **Live-verified** 2026-09-21 (2,206 real AMQERR01.LOG lines); replay fixture in our suite |
| Cert status shape (cipher/label/peer from PCF; expiry only when the driver exposes it) | **Live-verified** 2026-09-21 (plain TCP: nulls, as expected) |
| Privileged: channel restart, MAXDEPTH change, listener start, DLQ quarantine | **Live-verified** 2026-09-21 (real MAXDEPTH alter 5000->1000->5000, real stopped-channel restart, real listener start, real quarantine paths) |
| Separate privileged credential enforced; refused act() leaves the read session untouched | **Live-verified** 2026-09-21 |
| No secret in repr / exceptions / results | Verified with fakes in our suite |
| Record/replay fixtures from the live session | Committed: `tests/fixtures/recorded/ibmmq/` (9 fixtures); `tests/test_ibmmq_replay.py` replays them with no driver installed (11/11 pass) |
| **Live PCF against YOUR queue manager** (your queues, your channel status codes, your cert attributes) | Requires live pilot |
| **TLS handshake with your cipher + key repository** (only plain TCP exercised so far) | Requires live pilot |
...
### Live session notes (2026-09-21)

Validated against a real IBM MQ 10.0.0.5 queue manager (`QM1`, plain TCP,
throwaway developer queue manager with simplified local auth): **18/18
functional checks passed**, plus two deliberate error-path probes (unknown
queue -> clean `ConnectorError`; wrong queue-manager name rejected).
Seeded incident state: `APP.Q1` depth 60, `BACKLOG.Q` depth 200,
`BATCH.CHL` STOPPED. Seven real connector defects were found and fixed
during this session (PCF object-type parameters, listener command and
status codes, channel/listener restart reason-code tolerance, one-connection-per-thread
session handling, PCF byte-string decoding). Details in
`docs/LIVE_VALIDATION_2026-09-21.md`. Honest limits of this session: TLS
was not exercised; customer authentication, network policy, clustering,
and production topology remain unproven against real customer estates.
...
## Dev-machine live validation (2026-09-21)

Before any customer pilot, the real client code paths of 6 connectors were run
against real services on a dev machine (localhost, throwaway data; full
results in `docs/LIVE_VALIDATION_2026-09-21.md`; raw results in
`hidden_files/live-validation/*.json`). The static harness still cannot see
live targets — this run covered what it cannot.

| Connector | Live target | Result |
|---|---|---|
| `redis` | Redis 7.0.15 | 4/4 checks pass (INFO, replication, slowlog, seed round-trip) |
| `postgres` | PostgreSQL 16.15 | 4/4 checks pass (health, blocking, replication, seeded query) |
| `nginx` | nginx 1.24.0 | 2/2 checks pass (live stub_status counters, access-log parse) |
| `kafka` | Kafka 3.9.1 (KRaft) | 6/7 pass initially; `get_kafka_broker_config` failed against kafka-python 3.x (wrong `ConfigResource` import path + stale futures-dict API) — **fixed and re-validated: 314 configs, correct values, clean error on unknown broker** |
| `tomcat` | Tomcat 10.1.46 (Jolokia 2.6.3 + manager text) | 6/8 pass initially; `get_tomcat_threadpool` failed on bulk Jolokia GET read, `get_tomcat_apps` (jolokia) failed on over-URL-quoted mbean names — **both fixed and re-validated: real threadpool numbers (busy=1, count=10, max=200), all 6 apps `running` with real session counts** |
| `linux-host` | this machine | 5/5 pass (real metrics; allow-list and remote-host boundaries held) |

What this closes: "untested client code" for these 6 connectors. What it does
**not** close: your environment — TLS, auth schemes, network policy, your
versions, clustered topologies, and the 15 connectors not covered here all
still require the pilot below. The three bugs found above were fixed the same
day and re-validated live against fresh services; they are invisible to the
fake-transport suite, which is exactly why live validation runs before sign-off.

### Oracle DB: no live server on the dev machine (honest verdict, 2026-09-21)

We investigated running a real Oracle Database server on this dev machine
for record/replay fixture capture and concluded it is **not feasible
here**: Oracle Database Free ships RPM-only for Oracle Linux/RHEL (no DEB,
no tarball; verified against the current 26ai distribution list), the
RPM's own header declares a 3.83 GiB installed payload against 6.9 GiB
free disk (before any database is even created), there is no container
runtime on this box, and `container-registry.oracle.com` requires an
Oracle account plus license acceptance to pull (HTTP 401 verified). No
open-source server speaks Oracle's proprietary TNS wire protocol, so
there is no compatible substitute.

The `oracle_db` connector therefore remains **simulated /
fake-transport tested only** — honestly labeled, not quietly pretending.
Unblock options for real fixtures:

1. Oracle Cloud Always Free Autonomous Database (2 instances, 20 GB each,
   $0): sign up at oracle.com/cloud/free (credit card for identity check),
   provision one ADB, and connect with the connector's `oracledb` thin
   mode — no server footprint needed anywhere.
2. A Docker host with ~10 GB free disk plus a free Oracle account:
   `docker run -d -p 1521:1521 -e ORACLE_PWD=<pwd>
   container-registry.oracle.com/database/free:latest` (service `FREEPDB1`).
