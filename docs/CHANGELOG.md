...
- **Incident room (flagship demo):** `rca incident` serves a
  mobile-friendly incident console replaying ONE real incident through
  the REAL pipeline: queue manager QM1 (IBM MQ 10.0.0.5), channel
  `BATCH.CHL` stopped, 200 messages backlogged on `BACKLOG.Q` -- evidence
  recorded live on 2026-09-21 and committed as fixtures under
  `tests/fixtures/recorded/ibmmq/`. The genuine `IBMQConnector` read
  paths serve the fixtures in replay mode (no driver, no network), the
  `RCAEngine` diagnoses the new `mq_channel_backlog` alert type with
  cited evidence, Jev advises (honest deterministic fallback without a
  key), and the remediation plan's approval is REHEARSED
  (`executed: false, rehearsal: true` -- no privileged tool is even
  registered on this path). `POST /api/incident/verify` replays genuine
  post-recovery fixtures recorded live right after the real recovery
  (channel started, queue drained) under
  `tests/fixtures/recorded/ibmmq_recovered/`, and says so plainly when
  they are absent. New routes `/incident`, `/api/incident/{run,decide,
  verify,audit}`; `demo/incident.py` + `demo/live_estate.py`; zero-dependency
  frontend (`demo/static/incident.{html,css,js}`).
- **ibmmq `restart_channel` honesty fix:** after `MQCMD_START_CHANNEL`
  the connector now polls the real channel status (settle wait, ~10s
  timeout / ~0.5s interval) and reports the OBSERVED status instead of a
  hardcoded `"RUNNING"`. The result gains `polls` (number of status
  reads) and, when the channel never reaches RUNNING, a `note` such as
  `"channel settled at BINDING after 10s; not claimed RUNNING"`.
  Previously a live run sat at BINDING while the method claimed RUNNING.
  Privileged credential separation, the MQRCCF_CHANNEL_NOT_ACTIVE
  tolerance, and `_wrap` error handling are unchanged.
