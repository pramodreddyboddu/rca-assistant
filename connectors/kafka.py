...
class KafkaConnector(Connector):
    ...
    def get_kafka_broker_config(self, broker_id: int) -> dict:
        ...
        Config values are operational (not secrets), but credentials are
        still never logged. Sensitive entries surface as None, matching the
        driver's own redaction.
        """
        kafka = _kafka()
        ...
        try:
            ...
            future = futures.get(resource) if isinstance(futures, dict) \
                else None
            if future is None:
                raise ConnectorError(
                    f"{self.name}: describe_configs returned no future for "
                    f"broker {broker_id!r}"
                )
    ...
    def _broker_config_resource(self, kafka: Any, broker_id: int) -> Any:
        ...
        Tolerates driver layout differences (ConfigResource exported at the
        top level or under kafka.admin.config_resource); raises
        ...
        if config_resource is None or resource_type is None:
            ...
            cr_mod = getattr(admin_pkg, "config_resource", None) \
                if admin_pkg is not None else None
            if cr_mod is not None:
                config_resource = getattr(cr_mod, "ConfigResource", None)
                resource_type = getattr(cr_mod, "ConfigResourceType", None)
        if config_resource is None or resource_type is None:
            raise ConnectorError(
