...
class TomcatConnector(Connector):
    ...
    def _jolokia_read_mbean(self, mbean: str) -> dict:
        # POST-body reads: a GET with a fully-quoted ObjectName breaks on
        # stock Tomcat (allowEncodedSlash=false rejects the %2F%2F in
        # names like //localhost/orders with HTTP 400).
        resp = self._transport.post_json(
            self._jolokia_base,
    ...
    def _jolokia_read_attrs(self, mbean: str, attrs: list[str]) -> dict:
        """Read selected attributes via POST.

        A bulk full-attribute read is fragile: one throwing attribute
        getter (e.g. NioEndpoint.getDeferAccept on Tomcat 10.1) fails the
        whole read. Reading only the attributes we need avoids that.
        """
        resp = self._transport.post_json(
            self._jolokia_base,
            {"type": "read", "mbean": mbean, "attribute": list(attrs)},
            self._resolve_read_credentials(),
        )
        value = resp.get("value")
        if not isinstance(value, dict):
            raise ConnectorError(
                f"{self.name}: Jolokia attribute read of {mbean!r} "
    ...
    def get_tomcat_threadpool(self, pool: str | None = None) -> dict:
        ...
        pools: list[dict] = []
        for mbean in mbeans:
            # Per-attribute POST read: a bulk full-attribute GET fails the
            # whole pool when one attribute getter throws server-side.
            attrs = self._jolokia_read_attrs(mbean, [
                "name", "currentThreadsBusy", "currentThreadCount",
                "maxThreads",
            ])
    ...
    def _apps_via_jolokia(self) -> dict:
        ...
        apps: list[dict] = []
        for mbean in mbeans:
            # Attribute-scoped reads: a full-mbean read can 500 when any
            # single attribute getter throws server-side, and a multi-
            # attribute read 404s when any one attribute is absent — so
            # read the preferred attribute first, fall back to the legacy
            # name only when the mbean lacks it.
            attrs: dict[str, Any] | None = None
            last_err: ConnectorError | None = None
            for attr in ("stateName", "state"):
                try:
                    attrs = self._jolokia_read_attrs(mbean, [attr])
                    break
                except ConnectorError as exc:
                    last_err = exc
            if attrs is None:
                raise last_err or ConnectorError(
                    f"{self.name}: no state attribute on {mbean!r}")
            ...
            try:
                mgr = self._jolokia_read_attrs(
                    manager_mbean, ["activeSessions"])
