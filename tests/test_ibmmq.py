...
def _make_fake_pymqi(state, fail_connect=False):
    ...
    # Mirrors the real driver: channel/listener PCF constants live in
    # pymqi.CMQCFC (not CMQC), with the real numeric values. The three SSL
    # cert constants do not exist in the real driver at all; the fake keeps
    # them so the connector's graceful-None path for populated responses is
    # exercised.
    ...
    class FakeQueueManager:
        ...
        def connect_tcp_client(self, name, cd, channel, conn_name,
            ...
            # Mirrors the real pymqi >= 1.12 driver: the driver itself fills
            # in ChannelName/ConnectionName on the CD. Records everything
            # EXCEPT the password.
            cd.ChannelName = channel
            cd.ConnectionName = conn_name
            self.connect_calls.append({
                "kind": "connect_tcp_client", "qmgr": name,
                "channel": channel, "conn_info": conn_name, "user": user,
            })
            if fail_connect or "unreachable" in conn_name:
    ...
    class FakePCFExecute:
        ...
        def MQCMD_INQUIRE_CHANNEL_STATUS(self, args):
            ...
            return [{CMQCFC.MQCACH_CHANNEL_NAME: name,
                     ...
                     CMQCFC.MQCACH_SSL_CIPHER_SPEC: ch.get("cipher"),
                     CMQCFC.MQCACH_SSL_CERT_LABEL: ch.get("cert_label"),
                     CMQCFC.MQCACH_SSL_PEER_NAME: ch.get("ssl_peer"),
                     CMQCFC.MQIACH_SSL_CERT_VALID: ch.get("valid"),
                     CMQCFC.MQCACH_SSL_CERT_EXPIRY: ch.get("expires")}]

        def MQCMD_INQUIRE_LISTENER_STATUS(self, args):
            name = args[CMQCFC.MQCACH_LISTENER_NAME]
            if name not in state["listeners"]:
                raise FakeMQMIError(f"unknown listener {name!r}", comp=2,
                                    reason=2085, verb="INQUIRE_LISTENER")
            if state["listeners"][name] == 0:
                # Mirrors the live driver: a stopped listener has no status
                # instance, so PCF answers MQRC_UNKNOWN_OBJECT_NAME.
                raise FakeMQMIError(f"listener {name!r} stopped", comp=2,
                                    reason=2085, verb="INQUIRE_LISTENER")
            return [{CMQCFC.MQCACH_LISTENER_NAME: name,
                     CMQCFC.MQIACH_LISTENER_STATUS: state["listeners"][name]}]

        def MQCMD_INQUIRE_LISTENER(self, args):
            name = args[CMQCFC.MQCACH_LISTENER_NAME]
            if name not in state["listeners"]:
                raise FakeMQMIError(f"unknown listener {name!r}", comp=2,
                                    reason=2085, verb="INQUIRE_LISTENER")
        ...
        def MQCMD_STOP_CHANNEL(self, args):
            ...
            if ch["status"] == 6:  # already STOPPED: live server says 4064
                raise FakeMQMIError("channel not active", comp=2, reason=4064,
                                    verb="STOP_CHANNEL")
            ch["status"] = 6
            return []

        def MQCMD_START_CHANNEL(self, args):
            state["channels"][args[CMQCFC.MQCACH_CHANNEL_NAME]]["status"] = 3
            return []
        ...
        def MQCMD_CHANGE_Q(self, args):
            ...
            if CMQC.MQIA_Q_TYPE not in args:
                # Mirrors the live command server: QType is required.
                raise FakeMQMIError("QType required", comp=2, reason=3014,
                                    verb="CHANGE_Q")
            state["queues"][name]["max"] = args[CMQC.MQIA_MAX_Q_DEPTH]
            return []

        def MQCMD_START_CHANNEL_LISTENER(self, args):
            state["listeners"][args[CMQCFC.MQCACH_LISTENER_NAME]] = 2
            return []
...
def test_get_listener_status_stopped_listener_reports_stopped(fake_mq):
    # Live: a defined-but-stopped listener has no status instance (PCF
    # 2085); the connector must still report STOPPED, not raise.
...
def test_get_listener_status_unknown_listener_is_connector_error(fake_mq):
    conn = _conn()
    try:
        with pytest.raises(ConnectorError) as excinfo:
            conn.get_listener_status("NOPE.LISTENER")
    finally:
        conn.close()
    assert "unknown" in str(excinfo.value).lower()
...
def test_restart_channel_shape_matches_sim(fake_mq):
    ...
    try:
        ...
        conn.close()
    # The fake's start lands the channel at RUNNING immediately, so the
    # poll observes it on the first read; the method reports the observed
    # truth plus how many polls that took.
    assert set(d) == {"qmgr", "channel", "previous_status", "status",
                      "polls", "ts"}
...
def test_privileged_action_restores_read_session(fake_mq):
    # The driver allows one live connection per thread, so act() runs the
    # privileged call on a sequential admin session; the read session must
    # be re-established afterwards on the same manager object.
    conn = _conn()
    conn.connect()
    read_mgr = conn._mgr
    try:
        ...
        assert read_mgr.connected is True
        d = conn.get_queue_depth("QM1", "PAYMENTS.IN")
        assert d["depth"] == 48500
    finally:
        conn.close()
...
def test_start_listener_already_running_is_noop_success(fake_mq):
    # Live: the server answers MQRCCF_LISTENER_RUNNING; the connector
    # treats an already-running listener as a no-op success.
    conn = _conn()
    try:
        d = conn.start_listener("TCP.LISTENER")
    finally:
