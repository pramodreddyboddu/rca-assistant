...
class FakeTransport:
    ...
    def get_json(self, url, auth):
        ...
        if "/read/" in url:
            ...
            if "j2eeType=WebModule" in mbean:
                if "//localhost/reports" in mbean:
                    return {"value": {"stateName": "STOPPED"}}
                return {"value": {"stateName": "STARTED"}}
            if "type=Manager" in mbean:
                if "context=/orders" in mbean:
                    return {"value": {"activeSessions": 12}}
                if "context=,host" in mbean:  # ROOT context
                    return {"value": {"activeSessions": 3}}
                return {"value": {}}
