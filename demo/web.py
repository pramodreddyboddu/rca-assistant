...
from demo.incident import (audit_entries_json, decide_incident, run_incident,
                           verify_incident)
...
class _AppState:
    def __init__(self) -> None:
        ...
        # Incident room: single-incident state for /api/incident/*. None
        # until the first POST /api/incident/run. Only used when the
        # server was started with --incident-room.
        self.incident_room = False
...
def _handler_class(app: _AppState):
    ...
    class WebHandler(BaseHTTPRequestHandler):
        ...
        # -- API: incident room --------------------------------------
        # One REAL incident (BATCH.CHL stopped, BACKLOG.Q backlogged on
        # QM1) through the real pipeline over replayed fixtures. Approval
        # is rehearsed, never executed; verification replays post-recovery
        # fixtures when they exist.
        def _api_incident_run(self) -> None:
            response, audit, plan_record = run_incident()
            app.incident = {
        ...
        def _api_incident_decide(self) -> None:
            data = self._json_body()
            decision = data.get("decision")
            inc = app.incident
            if inc is None or inc["plan"]["decided"]:
                raise _HTTPError(
                    400, "no incident run pending approval: "
                         "POST /api/incident/run first")
            try:
                result = decide_incident(inc["plan"], decision)
            except ValueError as exc:
                raise _HTTPError(400, str(exc)) from exc
            self._send_json(200, result)

        def _api_incident_verify(self) -> None:
            self._send_json(200, verify_incident())

        def _api_incident_audit(self) -> None:
            inc = app.incident
            if inc is None:
                raise _HTTPError(
                    404, "no incident run yet: POST /api/incident/run first")
            self._send_json(200, {
                "mode": "replay-demo",
                "entries": audit_entries_json(inc["audit"]),
            })
        ...
        def _dispatch(self) -> None:
            ...
            if self.command == "GET":
                if path == "/":
                    # --incident-room: the incident page is the product;
                    # the sim index stays the default for `rca serve`.
                    # 404 JSON when incident.html is not installed yet.
                    if app.incident_room:
                        self._send_static("incident.html")
                    else:
                ...
                if path in ("/static/app.js", "/static/style.css",
                              "/static/incident.js", "/static/incident.css"):
                ...
                with app.lock:
                    ...
                    if path == "/api/host/compare":
                        ...
                        return
                    if path == "/api/incident/audit":
                        self._api_incident_audit()
                        return
            ...
            if self.command == "POST":
                if path == "/api/runs":
                    ...
                    return
                if path == "/api/incident/run":
                    with app.lock:
                        self._api_incident_run()
                    return
                if path == "/api/incident/decide":
                    with app.lock:
                        self._api_incident_decide()
                    return
                if path == "/api/incident/verify":
                    with app.lock:
                        self._api_incident_verify()
                    return
                m = _API_RUN_RE.match(path)
                if m:
                    run_id, action = m.group(1), m.group(2)
...
def make_server(port: int = _DEFAULT_PORT,
               bind: str | None = None,
               incident_room: bool = False) -> ThreadingHTTPServer:
    ...
    incident_room=True serves the incident-room page at / (the replayed
    real-incident flow in demo/incident.py) instead of the sim index.
    """
    app = _AppState()
    app.incident_room = incident_room
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--incident-room", action="store_true",
                   help="serve the incident room at /: one real incident "
                        "(BATCH.CHL stopped, BACKLOG.Q backlogged on QM1) "
                        "diagnosed through the real pipeline over replayed "
                        "fixtures, with rehearsed approval and verification")
    return p
...
def main(argv=None) -> int:
    ...
    server = make_server(port=args.port, bind=args.bind,
                         incident_room=args.incident_room)
    print("RCA assistant web demo")
    if args.incident_room:
        print("Incident room: real incident, replayed evidence")
