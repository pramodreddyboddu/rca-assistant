...
def _check_alert_channel_running(alert: dict, ev: dict[str, Any]):
    status, resp = _alert_channel_status(alert, ev)
    if status == "RUNNING":
        return True, Citation(
            claim=f"Channel {alert['channel']} is RUNNING",
            tool="get_channel_status",
            excerpt=_short(json.dumps(resp, sort_keys=True)),
        )
    return False, None
...
def _check_backlog_present(alert: dict, ev: dict[str, Any]):
    """The alert queue holds a real backlog (any depth above zero)."""
    depth, resp = _queue_depth(alert, ev)
    if _num(depth) and depth > 0:
        return True, Citation(
            claim=(f"Queue {alert['queue']} has {int(depth)} messages "
                   "backlogged"),
...
def _check_error_log_channel_inactive(alert: dict, ev: dict[str, Any]):
    """Error log records the alert channel as not currently active.

    Uses the no-limit evidence key: the recorded read_error_log fixture
    was captured without a limit argument, so the evidence plan calls
    read_error_log with {"qmgr"} only.
    """
    resp = ev.get(_ev_key("read_error_log", {"qmgr": alert["qmgr"]}))
    text = json.dumps(resp, sort_keys=True) if resp is not None else ""
    idx = text.find("not currently active")
    if idx < 0:
        return False, None
    return True, Citation(
        claim=(f"Error log records channel {alert['channel']} as not "
               "currently active (AMQ9533W)"),
        tool="read_error_log",
        excerpt=text[max(0, idx - 60):idx + 100][:_EXCERPT_LIMIT],
    )


def _check_contrast_channel_running(alert: dict, ev: dict[str, Any]):
    """A second, healthy channel as contrast: the fault is channel-specific."""
    resp = ev.get(_ev_key("get_channel_status",
                          {"qmgr": alert["qmgr"], "channel": "APP.SVRCONN"}))
    if isinstance(resp, dict) and resp.get("status") == "RUNNING":
        return True, Citation(
            claim=("Contrast channel APP.SVRCONN is RUNNING: the outage is "
                   "specific to the alert channel, not the queue manager"),
            tool="get_channel_status",
            excerpt=_short(json.dumps(resp, sort_keys=True)),
        )
    return False, None
...
def _remediation_restart_alert_channel(alert: dict,
                                       ev: dict[str, Any]) -> dict | None:
    """Build (not execute) the restart remediation for the ALERT channel.

    Unlike _remediation_restart_stopped (which scans the simulated
    _CANDIDATE_CHANNELS), this targets alert["channel"] directly: the
    incident-room alert names the real recorded channel.
    """
    status, _ = _alert_channel_status(alert, ev)
    if status == "STOPPED":
        return {
            "tool": "restart_channel",
            ...
            "requires_scope": "admin:write",
            "rationale": (
                f"Channel {alert['channel']} on {alert['qmgr']} is STOPPED "
                f"while queue {alert['queue']} has a backlog; restarting the "
                "channel should resume message flow. "
                "Requires human approval."
            ),
        }
    return None
...
def _plan_mq_channel_backlog(alert: dict) -> list[tuple[str, dict]]:
    """Evidence plan for the incident-room alert (real recorded fixtures).

    Every (tool, args) combo below must have a committed fixture under
    tests/fixtures/recorded/ibmmq/: read_error_log is called WITHOUT a
    limit argument because the recorded request's key_args carry no limit.
    """
    qmgr = alert["qmgr"]
    return [
        ("get_channel_status", {"qmgr": qmgr, "channel": alert["channel"]}),
        ("get_queue_depth", {"qmgr": qmgr, "queue": alert["queue"]}),
        ("read_error_log", {"qmgr": qmgr}),
        # Healthy contrast: APP.SVRCONN is recorded RUNNING.
        ("get_channel_status", {"qmgr": qmgr, "channel": "APP.SVRCONN"}),
    ]
...
_ALERT_HANDLERS: dict[str, tuple] = {
    ...
    # Incident-room alert: one REAL incident (BATCH.CHL stopped, BACKLOG.Q
    # backlogged on QM1) diagnosed through the REAL IBM MQ connector in
    # recorder replay mode against the fixtures recorded live on 2026-09-21.
    "mq_channel_backlog": (
        _plan_mq_channel_backlog,
        [
            _HypothesisSpec(
                "h1", "Channel stopped, backlog building",
                [_check_alert_channel_stopped,
                 _check_backlog_present,
                 _check_error_log_channel_inactive],
                _remediation_restart_alert_channel,
                None),
            _HypothesisSpec(
                "h2", "Upstream producer surge",
                [_check_backlog_present,
                 _check_contrast_channel_running,
                 _check_alert_channel_running]),
        ],
        (),
    ),
}
